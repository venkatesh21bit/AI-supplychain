...
import { useLogiBotAutoTrigger, useLogiBotEventListener } from '@/components/manufacturer/stockcount/LogiBotAutoTrigger';
...
export default function StockCountPage() {
  ...
  // 🤖 LOGI-BOT Auto-Trigger Integration
  const { triggeredProducts, resetTrigger } = useLogiBotAutoTrigger(stockData, {
    criticalThreshold: 10,
    checkInterval: 5000,
    enabled: true
  });
  
  // Listen for LOGI-BOT events
  useLogiBotEventListener();
    ...
    <div className="relative p-6 bg-black text-blue-300 min-h-screen">
      <h1 className="text-xl font-semibold mb-6 text-blue-400 flex items-center justify-between">
        <div className="flex items-center gap-4">
          Stock Dashboard
          {/* 🤖 LOGI-BOT Status Indicator */}
          <div className="flex items-center gap-2 text-sm">
            <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
            <span className="text-green-400">LOGI-BOT Active</span>
            {triggeredProducts.length > 0 && (
              <span className="px-2 py-1 bg-red-600 text-white text-xs rounded-full">
                {triggeredProducts.length} alerts triggered
              </span>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <StockOverview activeView={activeView} stockData={stockData} onUpdate={refetch} />
        </div>
