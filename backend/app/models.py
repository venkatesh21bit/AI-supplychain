...
# ==================== LOGI-BOT Agent Models ====================

class AgentAlert(models.Model):
    """Tracks alerts detected by LOGI-BOT agent."""
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='agent_alerts')
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='agent_alerts', null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.get_alert_type_display()} - {self.product.name if self.product else 'N/A'} ({self.status})"
...
class AgentExecution(models.Model):
    ...
    execution_id = models.CharField(max_length=100, unique=True, primary_key=True)
    alert = models.ForeignKey(AgentAlert, on_delete=models.CASCADE, related_name='executions')
    
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='started')
    
    # Execution details
    root_cause = models.CharField(max_length=100, null=True, blank=True)
...
class AgentWorkflowStep(models.Model):
    ...
    execution = models.ForeignKey(AgentExecution, on_delete=models.CASCADE, related_name='workflow_steps')
    ...
    def __str__(self):
        return f"{self.execution.execution_id} - Step {self.step_number}: {self.step_name}"
...
class AgentConfiguration(models.Model):
    """Stores agent configuration and thresholds per company."""
    
    company = models.OneToOneField(Company, on_delete=models.CASCADE, related_name='agent_config')
    ...
    def __str__(self):
        return f"Agent Config for {self.company.name}"